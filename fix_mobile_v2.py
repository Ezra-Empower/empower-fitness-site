#!/usr/bin/env python3
"""
fix_mobile_v2.py — Empower Fitness mobile fix (attempt 3)
Root causes addressed:
  1. Inline style width:750px / 960px etc. override media queries → replace with max-width
  2. grid-template-columns with fixed px → replaced with auto-fit
  3. Old injected CSS block removed and re-injected with corrected selectors
  4. Hamburger toggle JS added to every page
"""
...
# ── Regex patterns ─────────────────────────────────────────────────────────────
# Match old injected block: from the ═══ comment to end of the </style> it's in
OLD_BLOCK_RE = re.compile(
    r'/\*\s*═+\s*EMPOWER FITNESS[^*]*\*+/.*?(?=</style>)',
    re.DOTALL
)

# Match inline width:NNNpx where NNN >= 500 (not in a CSS rule context)
# e.g. style="width:750px" or style="... width: 960px ..."
INLINE_WIDTH_RE = re.compile(
    r'(style="[^"]*?)(?<!\-)width\s*:\s*([5-9]\d{2}|\d{4,})px([^"]*")',
    re.IGNORECASE
)

# Match grid-template-columns with px values inside style attributes
GRID_PX_RE = re.compile(
    r'(style="[^"]*?grid-template-columns\s*:\s*)([^"]+?)(")',
    re.IGNORECASE
)

def fix_inline_width(m):
    """Replace width:NNNpx with max-width:NNNpx;width:100% in inline styles."""
    before, px_val, after = m.group(1), m.group(2), m.group(3)
    return f'{before}max-width:{px_val}px;width:100%{after}'

def fix_grid_columns(m):
    """Replace px-based grid-template-columns with responsive auto-fit."""
    before, cols, after = m.group(1), m.group(2), m.group(3)
    # Count how many columns there are by splitting on spaces/commas
    # Replace px values: "300px 300px 300px" → "repeat(auto-fit,minmax(280px,1fr))"
    col_list = [c.strip() for c in re.split(r'[\s,]+', cols.strip()) if c.strip()]
    n = len(col_list)
    min_w = 260 if n >= 3 else 300
    return f'{before}repeat(auto-fit,minmax({min_w}px,1fr)){after}'

def already_has_hamburger_js(html):
    return 'nav-hamburger' in html and 'classList.toggle' in html
...
def process_file(path):
    ...
    if len(html) < 1000:
        return False, 'skipped (stub)'

    original = html

    # 1. Remove old injected CSS block
    html = OLD_BLOCK_RE.sub('', html)

    # 2. Fix inline width:NNNpx → max-width:NNNpx;width:100%
    html = INLINE_WIDTH_RE.sub(fix_inline_width, html)

    # 3. Fix grid-template-columns with px values in inline styles
    html = GRID_PX_RE.sub(fix_grid_columns, html)

    # 4. Re-inject mobile CSS before </style> (first occurrence)
    if '</style>' in html:
        html = html.replace('</style>', MOBILE_CSS + '\n</style>', 1)
    elif '</head>' in html:
        html = html.replace('</head>', f'<style>{MOBILE_CSS}</style>\n</head>', 1)

    # 5. Add hamburger JS before </body> if not already present
    if not already_has_hamburger_js(html) and '</body>' in html:
...
for path in sorted(html_files):
    ...
    if changed:
        fixed += 1
        print(f'  ✅ {rel}')
    elif reason == 'skipped (stub)':
        skipped += 1
    # else: unchanged — silent

print(f'\nDone: {fixed} fixed, {skipped} stubs skipped, {unchanged} unchanged')
